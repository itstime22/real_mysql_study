...
> 스터디 토론 내용
>
> InnoDB 테이블에서는 검색을 수행한 인덱스를 잠근 후 테이블의 레코드를 잠그는 방식으로 구현이 되어있음. 그래서, 인덱스를 잘 설정해줘야 불필요하게 레코드를 많이 잠그지 않아서 쓰기 성능을 높여줌
...
### 4. 커버링 인덱스가 무엇인가? (커버링 인덱스로 처리되는 쿼리는 디스크의 레코드를 읽지 않아도 됨)
